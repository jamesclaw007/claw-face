...
- Run windowed:
  - `claw-face --windowed`
- Run the GNOME/Wayland idle launcher (shows Claw Face when idle):
...
## GNOME Wayland Idle Behavior

1. Watch GNOME idle via DBus (`org.gnome.Mutter.IdleMonitor`)
2. Launch Claw Face fullscreen when idle
3. When Claw Face exits (ESC/Q), reset idle timer and wait for next idle
